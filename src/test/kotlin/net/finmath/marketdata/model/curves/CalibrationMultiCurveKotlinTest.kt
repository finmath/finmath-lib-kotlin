...
class CalibrationMultiCurveKotlinTest {
	...
	fun testSingeCurveCalibration() {
		...
		val deposit = { maturity: String, rate: Double ->
			...
			val scheduleInterfacePay: Schedule? = null
		 ...
		 */
		val discountCurveOIS = DiscountCurveInterpolation.createDiscountCurveFromDiscountFactors("discount-EUR-OIS", referenceDate, doubleArrayOf(0.0), doubleArrayOf(1.0), booleanArrayOf(false), CurveInterpolation.InterpolationMethod.LINEAR, CurveInterpolation.ExtrapolationMethod.CONSTANT, CurveInterpolation.InterpolationEntity.LOG_OF_VALUE)
		val forwardCurveOIS = ForwardCurveFromDiscountCurve("forward-EUR-OIS", "discount-EUR-OIS", referenceDate, "3M")
        val forwardCurve3M = ForwardCurveInterpolation("forward-EUR-3M", referenceDate, "3M", BusinessdayCalendarExcludingTARGETHolidays(),
			BusinessdayCalendar.DateRollConvention.FOLLOWING, CurveInterpolation.InterpolationMethod.LINEAR, CurveInterpolation.ExtrapolationMethod.CONSTANT, CurveInterpolation.InterpolationEntity.VALUE,ForwardCurveInterpolation.InterpolationEntityForward.FORWARD, "discount-EUR-OIS")
		val forwardCurve6M = ForwardCurveInterpolation("forward-EUR-6M", referenceDate, "6M", BusinessdayCalendarExcludingTARGETHolidays(), BusinessdayCalendar.DateRollConvention.FOLLOWING, CurveInterpolation.InterpolationMethod.LINEAR, CurveInterpolation.ExtrapolationMethod.CONSTANT, CurveInterpolation.InterpolationEntity.VALUE, ForwardCurveInterpolation.InterpolationEntityForward.FORWARD, "discount-EUR-OIS")

		val forwardCurveModel = AnalyticModelFromCurvesAndVols(arrayOf(discountCurveOIS, forwardCurveOIS, forwardCurve3M, forwardCurve6M))
		 ...
		 * Calibrate
		 */
		val calibratedCurves = CalibratedCurves(calibrationSpecs.toTypedArray(), forwardCurveModel, 1E-12)

        /*
         * Get the calibrated model
         */
        val calibratedModel = calibratedCurves.model
